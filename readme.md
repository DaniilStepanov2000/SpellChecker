...
1. Файл ```create_data.py``` - осуществляет парсинг страницы википедии, сохранение статьей в папку ```/data```, обработку текста, создание словаря с названием ```dictionary.txt``` и сохраняет его в папку ```/data```.
